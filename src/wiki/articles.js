...
export const ARTICLES = [
  ...
  page({
    ...
    figure: 'collide',
    air: [
      'You can land on the ground or on a roof. A gentle arrival settles the quad on the surface, where it can slide or come to rest. A fast arrival bounces, and a crash on the ground always ends with the quad lying flat, on its belly or on its back. A quad that comes to rest on its side is tipped over onto one or the other.',
    ...
    lab: [
      ...
      'The contact forces are found with an impulse method (accumulated impulses, eight passes per step). Friction is limited to the friction coefficient times the push of the surface, and the bounce depends on the speed of the impact. A quad pushed into a surface is moved back out by at most 5 cm per step. When a crashed quad lies slowly on the ground more than 14 degrees from flat, its rotors lose half their speed every millisecond, standing in for a pilot who would switch the motors off, and it is tipped towards whichever of belly or back is nearer. That tip is a chosen stand-in, not rigid body physics: a real quad rolls off its thin, springy props and arms, and the model\'s frame is a box that would otherwise stand on its side. Roofs count as ground: when the centre of gravity is above the top of a box that is higher than the ground, that top is the ground for the step. The train is a box moved every step by the map\'s own formula, so its position depends only on the step number.',
    ...
    source: 'src/native/world.c, src/native/sim.c TUMBLE FLAT, src/native/plant.c ground effect, src/game/plantworld.js, src/main.js stuckTick, src/game/collide.js (the set-down search)',
  }),
