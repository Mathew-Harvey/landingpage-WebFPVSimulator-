...
for (const axis of ['roll', 'pitch', 'yaw']) {
  ...
  put(`p_${axis}`, copy({
    ...
    air: `P is the immediate shove on ${a.axis}. Not ${a.motion} at the rate you asked, and P pushes ${a.mixer} harder in proportion to the shortfall. It is the gain people mean by snappy or lazy.`,
    lab: `Kp times (setpoint minus filtered gyro) on ${a.axis}, in firmware units, mixed onto the ${a.axis} column. P does not remember: a constant disturbance, nose-up at speed or motor cant, is I's job.`,
    ...
    upLab: `Higher Kp raises closed-loop bandwidth and shrinks tracking lag, and amplifies whatever noise survived the gyro filter. Damping ratio falls and D works harder. On yaw, extra P is often cheaper than extra D, because yaw D is noisy.`,
  ...
  put(`i_${axis}`, copy({
    ...
    air: `I is memory on ${a.axis}. When a rate error sits there, the nose rising at speed or a canted motor, I keeps pushing until it is gone. You meet it as trim you never hold.`,
    lab: `Integral of error. itermLimit, iterm_windup, iterm_relax and iterm_rotation are all LIVE and can stop or rotate the accumulator. Anti-gravity boosts I on fast throttle changes so a punch does not bow. A constant plant couple, H-force above the CG on pitch, is what this term cancels.`,
    ...
    upAir: `${a.Axis} holds a line better and fights drift you would otherwise trim out. Too much and the craft lurches when you stop a flip, unless relax is doing its job.`,
    upLab: `Higher Ki cuts steady-state rate error and winds faster. The unwind is the danger: a large accumulator dumped into the mixer is a twitch, which relax, windup percent and the limit exist to bound.`,
  ...
  put(`d_${axis}`, copy({
    ...
    air: `D is the brakes on ${a.axis}, opposing a rate error that is changing fast, which is how overshoot dies. It also hears every rattle in the gyro. Hence D's own filter, and hence a quiet sim making D look free.`,
    lab: `Kd times a filtered derivative of gyro, not of error, so a clean stick move is not punished. d_min_*, D max in the UI, keeps hover D low and raises it when the stick slams. TPA often attenuates D at high throttle. Yaw D is frequently low on real 5 inches.`,
    sim: `LIVE. Gyro hash in bf_glue.c is why this gain is not free here. Raise D, hear a buzz on punch, and cut less filter than you think, or you will export a tune that only works in a browser.`,
  ...
  put(`f_${axis}`, copy({
    ...
    air: `Feedforward on ${a.axis} starts the motors when you move ${a.stick}, before the gyro has had time to be wrong. It is the connected feeling, and the first thing a jittery radio ruins.`,
...
put('d_min_roll', copy({
  ...
  air: 'The D you want in a hover, when you are not slamming the stick. Betaflight\'s UI calls the high value D max; the CLI still calls the low one d_min. The firmware blends toward full D as setpoint acceleration rises.',
  lab: 'd_min[FD_ROLL] in the PID profile. d_max_gain and d_max_advance set how fast and how far D climbs from this floor toward d_roll, so hover D stays low and quiet while a flick still gets damping.',
...
function lpfPut(key, title, where, related) {
  put(key, copy({
    ...
    lab: 'Type is PT1, BIQUAD, PT2 or PT3; static Hz is the cutoff. Dynamic min/max/expo, when set, move that cutoff with throttle or the dynamic lpf logic in gyro_init and pid_init. A 1 kHz loop cannot cut what it cannot sample, so this is a phase-delay knob on the 0 to 500 Hz content that exists.',
...
put('gyro_notch1_hz', copy({
  ...
  downLab: '0 is off. Do not leave a 1 Hz notch sitting there from an experiment.',
}));
...
put('yaw_lowpass_hz', copy({
  ...
  air: 'Extra smoothing on yaw only. Yaw is the dirtiest axis, and this filters it without over-smoothing roll and pitch.',
  ...
  upAir: 'Raising Hz means less yaw filtering. For more smoothing, lower it.',
...
const RPM = (key, title, air, lab, upA, upL, downA, downL) => {
  put(key, copy({
    ...
    air,
    lab,
    sim: 'LIVE and compiled. getMotorFrequencyHz injects each rotor\'s Hz with telemetry-like lag, and what it can catch is harmonics of the once-per-rev imbalance lines. Blade passing is not in the glue, so the third harmonic guesses at a line that is not modelled.',
    upAir: upA,
    upLab: upL,
    downAir: downA,
    downLab: downL,
...
put('pid_process_denom', copy({
  ...
  upLab: 'Applied then overwritten. Not a way to make D easier.',
...
put('feedforward_averaging', copy({
  ...
  downAir: 'OFF: raw derivative. Sharp on Perfect, ugly on jitter.',
  downLab: 'OFF.',
}));
...
put('abs_control_gain', copy({
  ...
  air: 'Holds heading while you hold a roll, by feeding a yaw correction from the angle the craft has wandered. Racers often run it at 0; freestylers often want a little.',
...
put('use_integrated_yaw', copy({
  ...
  air: 'Treats yaw as an integral of motor difference rather than a direct torque demand. Can make yaw feel anchored, can also read as yaw lag. Off by default on most race dumps.',
...
put('dyn_idle_min_rpm', copy({
  ...
  air: 'The RPM governor\'s floor. A motor about to drop below it makes dynamic idle raise the mixer floor, which on a real ESC fights desync. This plant cannot desync, and the loop still runs on real rotor Hz.',
...
put('ez_landing_threshold', copy({
  ...
  air: 'Used when mixer_type is EZLANDING. How strong the landing cue must be before the mixer starts helping. Not the shell\'s landing judgement.',
...
put('rates_type', copy({
  ...
  air: 'BETAFLIGHT, RACEFLIGHT, KISS, ACTUAL or QUICK. The same three numbers mean different curves. ACTUAL is the one whose max rate is what it says at full stick, and the menu writes ACTUAL unless you change it on the FC screen.',
...
function rateAxis(axis) {
  ...
  put(`${axis}_expo`, copy({
    title: `${a.Axis} expo`,
    related: ['control-rates', `cli-${axis}_rc_rate`],
    air: `Bends ${a.axis} so the middle of the stick is gentler while the ends still reach max rate, on ACTUAL. 0 is linear in the ACTUAL sense, though not linear stick-to-rate if centre and max differ.`,
...
put('throttle_limit_type', copy({
  ...
  air: 'OFF, SCALE or CLIP. SCALE spreads the whole stick across 0 to cap, which is how racers lift hover off the floor on a 9 TWR machine. CLIP ignores the top of the stick.',
...
put('min_check', copy({
  ...
  upAir: 'A larger zone counted as minimum.',
...
function family(field) {
  ...
  if (k.startsWith('failsafe_')) {
    return copy({
      title: k,
      air: 'What the craft does if the radio dies: drop, land, GPS rescue. The link here fails only if you model loss in the radio preset, and even then there is no failsafe.c.',
    ...
    sections: [
      {
        ...
        title: 'In the air',
        paras: [
          'Every setting the real flight controller has, all 696, in the order a pilot sees them in the configurator. Green rows change how this aircraft flies. Grey rows are hardware this simulator does not have, a video transmitter or GPS, shown rather than hidden so pasting in a real settings file loses nothing. Open a key for what it does, what raising it feels like, and whether it does anything here.',
        ],
      },
      {
        id: 'lab',
        title: 'In the lab',
        paras: [
          `catalog.js is the status authority, bf_settings.c the write table, and pid.c, mixer.c, rc.c, gyro.c and rpm_filter.c the readers. A LIVE key with no reader is a bug. A family page for an INERT key is not: OSD has hundreds of coordinates and none of them fly.`,
        ],
      },
      {
        id: 'sim',
