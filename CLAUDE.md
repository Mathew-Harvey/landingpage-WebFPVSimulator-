...
**The simulator's code is copied, never edited, into `src/sim/`.** `src/sim/` mirrors the simulator's own `src/` layout, so every file arrives byte for byte and no import is rewritten. `node scripts/vendor.js ../WebFPVSimulator` makes it: the vendored town directory whole, and the import closure of the entries listed in that script. `src/sim/MANIFEST.json` records the simulator's commit and a SHA-256 per file, and `npm run lint:page` fails on any file that does not match. A change the page needs in that code is made in the simulator, which is the copy of record, and recopied.

**The town is the simulator's town.** `src/sim/maps/city/vendored/` is sakura-crossing, MIT, copied whole from the simulator's `src/maps/city/vendored/` and recorded in `NOTICE`. `src/city.js` is a join and nothing else: which parts of the town are built, and the line flown through it. Where it stands is `src/places.js`, because the page needs those numbers before the town exists. A hand built portrait of the town lived there first and was replaced, because a front door that advertises a place should show the place.

**The town is merged by the simulator's `bakeCity`, built and baked at the world origin and moved after.** It once looked as if `bakeCity` ate the shopping street's buildings, and a hand rolled merge replaced it; the bug was the caller's (`7f1dc8b`): the bake folds each mesh's world matrix into its vertices and parents the result back under the root, so a town standing 460 m away gets its offset twice. So the group sits at the origin for the build and the bake and takes its place at the end.

**Nothing heavy is built behind the boot screen.** It lifts on the studio's first frame. The town, its merge, and every place's warm pass (shaders compiled with `compileAsync`, buffers uploaded a slice at a time into an eight pixel scissor) are jobs in `src/loader.js`, run a step at a time by the budget in `main.js`: nothing while the visitor scrolls or the opening build plays, a little when the frame is quiet, more when it is still, and everything while a hold is waiting. The steps exist because the simulator's `buildWorldSteps` and `bakeCitySteps` are generators, hashed identical to the single calls. A place the film reaches before it is built is waited for inside its transition, the dissolve held in haze or the blackout held dark, with a note, never shown half built. A new place is a new job and a new hold, not a longer boot.
...
**The page credits Betaflight by name on the first screen, links them, and says to support them.** Act one names Betaflight where the control loop is first claimed, links `betaflight.com`, and says whose firmware is doing the hard part. It is not decoration: the control loop IS Betaflight compiled to WebAssembly, this page's whole claim rests on their work, and the licence this repository ships under is theirs by descent. The footer link is not enough on its own, because nobody scrolls to a footer to find out who wrote the thing they are watching fly.

**No link goes in a beat.** `#beats` is `pointer-events: none`, because it is fixed over the middle of a page whose whole interaction is the scroll, and a beat is on screen for a fifth of an act. A link in one would look clickable and not be. Anything that has to be clicked belongs in an act's copy block, the top bar, or the footer.
